...
def set_seed(seed):
    """
    Set random seeds comprehensively to ensure reproducibility
    """
    random.seed(seed)
    np.random.seed(seed)
    ...
    # Deterministic setting for matrix multiplication in CUDA 10.2+
...
def get_metrics(y_true, y_pred):
    ...
    # [Modification] Add float() casting here to fix np.float64 display issue
    return [
